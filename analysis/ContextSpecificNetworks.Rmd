...
foreach(item = c("regulonkirc", "regulonhnsc", "regulonlusc", "regulonluad", 
"regulonpaad")) %dopar% {
...
```{r}
for (item in c("regulonkirc", "regulonhnsc", "regulonlusc", "regulonluad", 
"regulonpaad")) {
    if (!file.exists(paste("output/regulon/", item, ".adj", sep = ""))) {
        ...
        # Group by Regulator and concatenate elements of each group
        # into strings
        df$Regulator <- paste(df$Regulator,"_P", sep = "")
        df$temp <- paste(df$Target, df$MoA, sep = "\t")
        result <- aggregate(temp ~ Regulator, data = df, FUN = function(x) paste(x,
            collapse = "\t"))
        # Print the result
        file <- file(paste("output/regulon/", item, ".adj", sep = ""))

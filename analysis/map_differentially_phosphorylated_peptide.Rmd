---
title: " Identify Known kinases from mapped differentially phosphorylated peptide"
output:
...
for (i in c(1, 3, 5, 7, 9)) {
    ...
    geneID <- ensembldb::select(EnsDb.Hsapiens.v86, keys = meta_normal$ENSG,
        keytype = "GENEID", columns = c("SYMBOL", "UNIPROTID", "GENEID"))
    meta_normal$UNIPROTID <- plyr::mapvalues(meta_normal$ENSG, from = geneID$GENEID,
    ...
    geneID <- ensembldb::select(EnsDb.Hsapiens.v86, keys = meta_tumor$ENSG,
        keytype = "UNIPROTID", columns = c("SYMBOL", "UNIPROTID", "GENEID"))
    meta_tumor$UNIPROTID <- plyr::mapvalues(meta_tumor$ENSG, from = geneID$GENEID,
        to = geneID$UNIPROTID, warn_missing = FALSE)
    rownames(meta_tumor) <- rownames(exprs_t)
    
    common_terms <- intersect(rownames(meta_tumor), rownames(meta_normal))
    
    exprs_t[!is.finite(as.matrix(exprs_t))] <- 0
    exprs_n[!is.finite(as.matrix(exprs_n))] <- 0
    
    expr <- cbind(exprs_t[common_terms, ], exprs_n[common_terms, ])
    
    saveRDS(exprs_n, paste("output/phos/count_matrix_",labels[i],"_normal.RDS",sep = ""))
    saveRDS(exprs_t, paste("output/phos/count_matrix_",labels[i],"_tumor.RDS",sep = ""))
    ...
    fc <- cbind(meta,fc)[(fc >= 2) & (fc$Adjusted_P_Value < 0.1), ]

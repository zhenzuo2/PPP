---
title: " Differentially Proteomics"
output:
...
dir.create("output/DAP/",showWarnings = FALSE)
dir.create("output/prot/",showWarnings = FALSE)
```

# Define functions
```{r}
read_prot <- function(file_name) {
...
# Extract and process Protein data
...
for (i in c(1, 3, 5, 7, 9)) {
    normal <- read_prot(df$prot[i])
    expr_n <- normal[[1]]
    meta_n <- normal[[2]]

    tumor <- read_prot(df$prot[i+1])
    expr_t <- tumor[[1]]
    meta_t <- tumor[[2]]
    ...
    saveRDS(expr_n[common_terms, ], paste("output/prot/count_matrix_", labels[i], "_normal.RDS",
        sep = ""))
    saveRDS(expr_t[common_terms, ], paste("output/prot/count_matrix_", labels[i], "_tumor.RDS",
        sep = ""))
    ...
    fc <- cbind(meta, fc)
    write.csv(fc, paste("output/DAP/", labels[i],
        "_fc.csv", sep = ""), row.names = F)

    network <- read.csv("data/KSA_gold_standard.csv")
    network <- network[network$Gene.name %in% fc$SYMBOL, ]
    unique(network$Gene.name)
    write.csv(fc[fc$SYMBOL %in% network$Gene.name, ], paste("output/DAP/", labels[i],
        "_fc_kinases.csv", sep = ""), row.names = F)
    write.csv(network, paste("output/DAP/", labels[i], "_KSA_gold_standard_0.05.csv",
        sep = ""), row.names = F)
}
```

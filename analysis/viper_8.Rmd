...
foreach(name = c("kirc", "hnsc", "lusc", "luad", "paad")) %dopar% {
    ...
    mrs <- msviper(signature, regulon, nullmodel, verbose = FALSE, minsize = 5)
...
```{r}
foreach(name = c("kirc", "hnsc", "lusc", "luad", "paad")) %dopar% {
  exprs <- read.table(paste("output/exprs/",
        name, "/exprs.txt", sep = ""))
  phos <- read.table(paste("output/phos/",
        name, "/phos.txt", sep = ""))
  cnv <- read.table(paste("output/cnv/",
        name, "/cnv.txt", sep = ""))
  prot <- read.table(paste("output/prot/",
        name, "/prot.txt", sep = ""))
  methy <- read.table(paste("output/methy/",
        name, "/methy.txt", sep = ""))

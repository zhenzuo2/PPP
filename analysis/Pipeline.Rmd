...
## To do
Fiter phosphorylated/proteomics peptide showing in at least 20 samples.   
change Phosphorylated Peptides to gene name + amio acid + location to index.  

# Pipeline for Omic Dataset Integration

## Find Differentially Phosphorylated Peptides
- Get proteomics data (and process). 
- Get phosphorylation data. 
- Replicates were averaged. 
- Run t-test (use WC test) on naive and benign samples to identify differentially phosphorylated peptide and proteins (FDR < 0.1, or 0.05 ). 
- Filter diffetingally phospho sites if p value < host protein level p value. 
- Select Differentially expressed kinases from proteomics data.  
- optional: kinases activation site overlapping with differentially phosphoal sites.  
- Run Kinase substrate enrichment analysis (KSEA) to find significant kinases. Save it and pass to the next step.  ranked list and prior knowldege (gmt files) ->  kinase activity score.

### Output
- Known kinases from mapped differentially kinase activity score. 
